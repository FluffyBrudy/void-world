...
class DotParticle(Particle):
    ...
    def render(self, surface: Surface, offset: Tuple[int, int] | Vector2):
        draw_circle(
            surface, self.color, self.pos - offset, self.radius, self.fill_width
        )


def coned_particles(
    pos: TPosType, base_angles: Sequence[float], group: "ParticleManager", filled=True
):
    for base_angle in base_angles:
        angle = base_angle
        if base_angle != 0:
            angle = base_angle + pi / 6
        radius = randint(6, 12)
        ...
        particle = DotParticle(
            ...
            pos,
            velocity,
            color=(randint(200, 255), randint(50, 80), randint(0, 50)),
            filled=filled,
        )
        group.add(particle)
